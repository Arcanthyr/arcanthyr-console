...
## Session 101 decisions — 25 April 2026

**AustLII CF-edge block confirmed — jade.io as replacement fetch source**
AustLII now returns HTTP 403 for all Cloudflare edge fetches (sinosrch, viewdoc, listing pages). Confirmed via Quick Search tab returning "AustLII returned HTTP 403" and firecrawl reachability test. Block extends beyond the VPS TCP block documented sessions 85/88 — now covers both infrastructure paths. `search_by_citation` also dead (VPS TCP-blocked). Two-step search pattern fully dead. Local scraper (residential IP) is the only working AustLII access path. jade.io is accessible from CF edge and serves identical judgment content at `jade.io/au/cases/tas/COURT/YEAR/NUM`. `handleFetchJudgment` now translates AustLII viewdoc URLs to jade.io for the live fetch while retaining the AustLII URL as the D1 cache key — intentional decoupling for cache stability. Quick Search word-search accepted loss (jade.io search is POST-based, no clean CF-edge replacement).

**fetchCaseContent BR path — design rationale**
viewdoc URLs 403 from CF edge even with browser-mimicking headers. Fix: `@cloudflare/puppeteer` via `[browser]` binding (wrangler.toml + `nodejs_compat` flag). Viewdoc URLs routed through puppeteer when `env.BROWSER` available; non-viewdoc URLs fall through to `handleFetchPage`. First deploy failed on missing `nodejs_compat` — required on first `@cloudflare/puppeteer` deploy.

**fetchRecentAustLIICases — root cause was structural, not header-based**
Initial brief described the bug as "dead VPS proxy path." Incorrect — the function already used direct CF-edge fetch with browser headers identical to `handleAustLIIWordSearch`. Actual problem: function probed sequential viewdoc URLs (`/cgi-bin/viewdoc/...`) which 403 regardless of headers. Fix: replace sequential probing loop with sinosrch-based discovery (reusing `parseAustLIIResults`), returning `{citation, url, court}` with no html so `fetchCaseContent` BR path handles content extraction. Lesson: read the function before briefing the fix.

**Roadmap items removed as noise**
Model swap evaluation (GLM/Qwen3 vs GPT alternatives), RRF, legal-vocab dictionary, oracle top-20 study, Pass 4 authority_synthesis second quota block, CONCEPTS vocabulary contamination watch item — all removed. Rationale: no real-use failure signal exists (zero `query_log.sufficient=0` rows); these items add maintenance surface without providing a trigger condition. Re-raise from real-use signals only.
...
### Session 104 decisions — 26 April 2026

- **Worker.js → worker.js rename** — renamed via two-step `git mv` (temp name required on Windows case-insensitive FS for case-only renames). Rationale: capital-W footgun had permanent entries in both SESSION RULES and KNOWN ISSUES; one rename eliminates ongoing vigilance tax indefinitely. wrangler.toml `main` was already lowercase — no toml change needed.
- **/research direct-nav 404 — canonical SPA catch-all pattern** — `env.ASSETS.fetch(request)` threw (no ASSETS binding); adding `binding = "ASSETS"` to `[assets]` enabled it but `not_found_handling = "single-page-application"` applies only to platform-level asset serving, not programmatic `.fetch()` calls. Final pattern: `env.ASSETS.fetch(new Request(new URL('/index.html', request.url)))` — explicitly request `/index.html`, bypassing `not_found_handling` entirely. This is the canonical SPA catch-all for Cloudflare Workers Assets.
- **Phase 2 route consolidation** — `/upload`, `/compose`, `/health-reports` removed as standalone routes; content absorbed into CorpusAdmin sub-tabs. Rationale: consolidates admin surface into one top-level tab, reduces nav clutter. Old page files retained temporarily as dead code — Phase 3 extracts inner content before deletion.
- **Fresh CC instance vs. compress — guidance formalised** — sequential task chains with shared state should compress and continue; fresh instances require full discovery context to be reconstructed in briefs (error-prone, token-heavy). Fresh instances appropriate for genuinely independent tasks only. Phase 2 Tasks 2–5 were sent to a fresh instance, requiring full discovery reconstruction — this was the wrong call; compress would have been cleaner.

### Session 106 decisions — 26 April 2026
...
### Session 109 decisions — 3 May 2026

- **Case Search list/detail split** — handleLibraryList was producing 12.2 MB D1 responses (6 full enriched-text columns × 1,914 rows + 2 correlated subqueries per row). Fix: strip list query to metadata only (citation, court, decision_date, subject_matter, deep_enriched, chunk counts via LEFT JOIN); new handleCaseDetail handler for single-row full-text fetch on case click. New route: GET /api/legal/case-detail. Payload 12.2 MB → 1.18 MB; intermittent 500 eliminated. The correlated-subquery anti-pattern (SELECT COUNT(*) inline in multi-row SELECT) is now confirmed non-recurring — only handleLibraryList had it at scale.
- **D1 list query payload rule adopted** — safe threshold: metadata-only columns, flat JOIN, result < ~3 MB. Over ~5 MB is in 30s Worker wall-clock timeout risk territory depending on D1 load. Intermittent 500s on a Worker list endpoint are almost always a payload/timeout issue, not a VPS or Qdrant issue — check (columns × avg_bytes × row_count) first.
- **Drift audit this session** — 7 cross-file MD fixes applied (c483e0a). Session-closer skill hardened with explicit datestamp verify step for both CLAUDE.md and CLAUDE_arch.md headers.

### Session 110 decisions — 3 May 2026

- Clerk Allowlist rejected (cost) — Allowlist (Protect → Restrictions → Allowlist) requires Clerk Pro plan; Arcanthyr is on Hobby. Email gate implemented as APPROVED_EMAILS constant in worker.js + verifyClerkToken JWKS verification instead. Trade-off: requires wrangler deploy to add/remove users; acceptable for 4–6 person closed tool.
- Clerk Restricted Mode rejected (UX) — Restricted Mode disables self sign-up entirely, requiring manual user creation. Tom's preference: open sign-up form, gate enforced by Worker email check. Allowlist in code achieves the same security outcome.
- AuthGate over ClerkTokenInitializer — ClerkTokenInitializer as a sibling node to Routes called initApi synchronously but Clerk's getToken returns null until isLoaded=true, causing first-render API calls to fire with no token. AuthGate wraps Routes, returns null until isLoaded, then calls initApi synchronously — guarantees token is wired before any child effect fires.
...
## Session 115 decisions — 9 May 2026

**RRF fusion reinstated after session 42 revert.** Session 42 reverted RRF because Leg B used the same embedding model on a munged query — no independent signal. Session 115 RRF fuses cosine semantic against FTS5 BM25 over an inverted-index, which is genuinely independent (different scoring algorithm, different index structure, different recall profile). Empirically confirmed by session 1 instrumentation showing 5/8 novel-hit rate from FTS leg on test query — the legs diverge usefully. Session 42 reasoning does not apply.

**LIMIT_PER_TABLE=24 chosen for fts_leg().** Old `fetch_case_chunks_fts` capped at 8 — too tight for compound-term queries with phrase operands plus singletons. 24 per surface × 3 surfaces = 72 candidate pool pre-fusion. Session 1 instrument later observed `fts_keyword_hits=72` on test query — full saturation, indicating the cap is doing real work.

**`min(2, available)` FTS-only quota floor.** Even with RRF, on queries where semantic and FTS strongly diverge a pure RRF fusion can produce a softer recurrence of H2 (semantic-dominated top_k with FTS chunks just below cap). Min-quota floor guarantees ≥2 FTS-only slots when FTS has unique candidates. Session 115 verification observed `rrf_quota_filled=0` on both verification queries — natural RRF was sufficient — but the floor is insurance against query distributions we haven't seen yet.

**`USE_FTS_LEG` feature flag.** Biggest retrieval change since session ~94. Flag enables instant rollback to semantic-only via env edit + `docker compose up -d --force-recreate agent-general`, with no Worker redeploy. Cost: one env var read at function start. Acceptable.

**No SM penalty on FTS-leg-only candidates.** Relationships Act 2003 is not on `LEG_WHITELIST_CORE` or `LEG_WHITELIST_ADJACENT` and would be penalised at SM_PENALTY=0.65 under semantic-leg logic. But it is the canonical authority for "significant relationship" in criminal contexts (s 167B Criminal Code references it). The SM whitelist is incomplete and expanding it ad hoc risks new penalties. Cleaner to apply SM penalty only to the semantic leg; FTS leg's BM25 ranking on phrase match is already a relevance filter.

**Three-FTS-surface architecture.** `case_chunks_fts` and `secondary_sources_fts` already existed. `legislation_sections_fts` created session 115 specifically because the named-driver query class needed access to definitional anchors that exist only in `legislation_sections` (not in any FTS surface previously). Definitional anchors are a distinct retrieval class: a single chunk per term, perfect phrase match, low cosine score because the section text is short and dominated by terminology. Without FTS access they are systemically unreachable for any query that doesn't include a section reference.

**FTS query construction order: literal phrases → bigram shingles → singletons.** Literal-quoted segments (`"significant relationship"`) are explicit user intent and become phrase MATCH operands. Bigram shingles approximate noun-phrase recall without an NLP layer. Singletons provide recall floor. All OR-joined: BM25 ranks by aggregate match strength. Length cap removed — original `[:8]` cap was the H1 root cause.

**`bm25_fired` column kept for historical continuity.** Renaming or repurposing breaks audit trail of every query before session 115. Added `fts_keyword_*` columns alongside instead. `bm25_fired` semantics remain: section-ref path only, regex `\bs\s*(\d+[A-Za-z]*)`.

**Score scale on `result_scores` shifts to RRF range (~0.013–0.017).** Previously a cosine/0.50-constant mix. Score-scale incompatibility was the proximate cause of H2 — eliminating shared scale eliminates the failure class. Frontend display polish deferred. Note: cosine-scale comparisons across query_log entries pre/post session 115 are no longer meaningful.

**Residual: verbose-query phrase-operand weighting in BM25 OR-join.** Q1 (verbose quoted query) returned Relationships Act s 4 at rank 3 but did not surface any of the three retagged cross-domain cases in top-5. Diagnosis: the OR-join puts `"significant relationship"` alongside ~14 other operands (singletons + bigram shingles), and BM25 rewards multi-operand matches. A chunk hitting `tas + cases + list + discuss` can outrank a chunk hitting only `"significant relationship"`. Not H1, not H2 — a third-tier ranking-weight question. Cheap fixes exist (FTS5 column weights, NEAR operator, score boost for chunks containing the literal phrase) but reserved for a future session if real-use queries cluster on this class.

**Dead end of historical interest: `bm25_fired` column was never a reliable diagnostic for FTS keyword path activity.** Pre-session-115, every retrieval analysis that read `bm25_fired=0` as "FTS keyword path didn't run" was wrong. The FTS keyword path ran unconditionally; `bm25_fired` recorded only the section-ref path. Future analyses must use `fts_keyword_*` columns.

## Session 116 decisions — 10 May 2026

**MD drift management — periodic reconciliation accepted as steady-state.** Across ~115 sessions, Tom has run three or four major reconciliation passes worth of drift. Each takes about an hour. That is <1% of session time as maintenance overhead. Building a more rigorous doc-as-data setup (drift detector, single-source-of-truth restructuring, per-session D1 assertions) would have real engineering cost on top of feature work, with cost-benefit not justified at current drift rate. Accepted approach: periodic reconciliation passes (10-15 sessions cadence) plus four cheap interventions queued for next session — `*.md text eol=lf` in `.gitattributes`, match-count assertion in any Python rewrite script, SYSTEM STATE refresh script (auto-emit markdown table fragment from D1 query at session open), close-procedure verify step grepping for strict descending session-number order in CLAUDE_changelog.md.

**FROZEN block re-opening — systemic-gap identification accepted as trigger.** Original freeze (session 96) specified "named real-use trigger from D1 query_log where sufficient=0" as the only re-opening signal. Session 114 identified a systemic compound-term retrieval gap empirically (without sufficient=0 logging) and session 115 shipped FTS leg + RRF as the fix. Refreeze is now gated on 31-query baseline re-run, not on sufficient=0 logging window expiry. Position: systemic gaps identified through architectural analysis are a legitimate parallel trigger to real-use feedback. Document this distinction explicitly in the FROZEN block (done this session).

**[2022] TASSC 69 retag drift — silent MCP UPDATE failure class.** Session 51 changelog described retagging [2022] TASSC 69 (DPP v Greenham Tasmania Pty Ltd) administrative to criminal via D1 MCP UPDATE. Session 116 D1 spot-check showed administrative — the change had not persisted (or was reverted by a later poller/merge run; root cause undetermined). Re-applied this session + Qdrant payload patched. Position: any future MCP UPDATE described as "complete" should immediately re-SELECT the row and confirm before being noted in changelog. One additional MCP call, prevents the entire class.
...
## Session 119 decisions — 12 May 2026

- **Extend session-101 jade URL translation to handleFetchCaseUrl.** AustLII is CF-edge blocked by Bot Management (session 102); the session-101 viewer used jade.io as a workaround. The URL upload path was failing for the same reason and the same workaround applies. Lower-cost than building a new fetch infrastructure (residential proxy, CF Tunnel, etc.) and reuses an already-validated path.

- **Path A (extract-text endpoint) over Path B (extend process-document) for Task 2 drag-and-drop with RTF.** process-document is built for the secondary-source chunking model (split text → per-block GPT enrichment → many small D1 inserts). Cases require a different downstream pipeline (Queue → Pass 1 → chunker → enrichment → merge → embed via poller). Forking process-document with a `target=cases` flag would duplicate or compromise one of the two pipelines. A lightweight extract-text endpoint that only returns text leaves both pipelines clean and reuses server.py's existing python-docx/pypdf/striprtf extractors. Day-one RTF support, DOCX and PDF arrive for free when the file-input `accept` attribute is widened.

- **Defer residential proxy / VPN workaround for AustLII bot blocking until scraper repair complete and need proven.** Considered: (a) paid residential proxy ($30–80/month, ~5 lines of code in server.py), (b) home Windows machine as egress via Cloudflare Tunnel (free, ~30 lines + cloudflared install). All commercial VPN ASNs are blocked by CF Bot Management; Tor exits are blocked; the "VPN to mask VPS IP" intuition does not work for this specific filter. The scraper already uses the residential-IP egress that would be replicated; once the scraper is repaired tonight, TASMC ingestion is restored on its primary path and URL upload becomes convenience-only. Build infrastructure only if real-use friction proves the demand after scraper repair. The home-machine-as-egress path also creates an invisible dependency on home internet uptime that could create confusing failures during real practitioner work.

- **Ship Task 1.5 (defensive checks) alone rather than combined with Task 2.** Separate deploy workflows (Worker-only vs Worker + server.py + frontend rebuild), Task 2 has unresolved design questions on form UX and citation/court source, and the scraper fix was tonight's planned work. Combining would have pushed scraper to next session and increased blast radius on the urgent defensive fix.

- **Dead end: jade.io is not a reliable TASMC fetch source.** Coverage gap is structural, not transient. TASMC URL upload via jade is permanently not viable. Do not retry. Use scraper or future drag-and-drop path for TASMC.

- **Dead end: `mcp__hex-ssh__ssh-write-chunk` on Windows.** Fails with `BAD_PATH: absolute remote path required, got "C:/Program Files/Git/home/tom/..."` — the tool resolves remote home directory through the local Windows Git Bash path and corrupts it. Do not retry. Use the base64-pipeline pattern via `remote-ssh` instead (see CLAUDE_init.md).
