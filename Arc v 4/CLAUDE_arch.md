...
*Updated: 20 April 2026 (end of session 80). Upload every session alongside CLAUDE.md.*
...
4. **Pass 4 — citation authority agent (shadow mode)** — gated by `should_fire_pass4(query_text)`: (a) keyword match in `AUTHORITY_KEYWORDS` (treatment vocab + citation-profile vocab + narrow topical phrases); (b) query ≤60 chars AND ≥1 citation (bare-lookup); (c) ≥2 citations (relationship intent). When gate fires and `AUTHORITY_PASS_ENABLED=true`: `type=authority_synthesis` must filter, `quarantined=True` must_not, threshold 0.50, limit 3, 500ms ThreadPoolExecutor timeout. Deduped against `seen_ids`. Currently **shadow mode** (`AUTHORITY_PASS_ENABLED=false` default) — gate logs but skips Qdrant query.
...
- Authority synthesis type: field = `type`, value = `authority_synthesis` — **LIVE** (session 79 ingest; session 80 Pass 4): 233 chunks ingested and embedded. Normal retrieval (Pass 1, Pass 3) blocked via `must_not` filter. Pass 4 gate (`should_fire_pass4()`) deployed in shadow mode (`AUTHORITY_PASS_ENABLED=false` default) — gate fires and logs but skips Qdrant query until flag flipped. Flip: add `AUTHORITY_PASS_ENABLED=true` to `~/ai-stack/.env.config` + force-recreate agent-general after telemetry review.
...
- **Citation authority agent (xref_agent.py)** — Pass 4 deployed session 80 in shadow mode. `should_fire_pass4()` gate (keyword + bare-lookup + multi-citation rules), `AUTHORITY_PASS_ENABLED=false` default, 500ms timeout, dedup via `seen_ids`. Worker `[AUTHORITY ANALYSIS]` labels + systemPrompt instructions live (version 648207f6). UI: amber AUTHORITY tag in ResultCard, Library badge, AuthorityPane in ReadingPane. Outstanding: flip `AUTHORITY_PASS_ENABLED=true` after shadow-mode telemetry review (monitor `[Pass 4]` log lines for false-positive topical-keyword FIRE rate).
