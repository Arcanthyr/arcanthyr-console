...
Updated: 19 April 2026 (end of session 75) · Supersedes all prior versions
...
| Baseline (31 queries) | 26P / 3Pa / 2M — session 74 (19 Apr 2026) · post-BM25-interleave deploy · canonical file: ~/retrieval_baseline_post_interleave.txt (generic ~/retrieval_baseline_results.txt is stale Apr 16, do not grep it) |
...
1. **Practitioner↔statutory vocabulary aliasing — scope narrowed to Q12 + Q23 only** — D1 verification session 75 confirmed only two of the four session-74 candidates have target chunks in corpus: Q12 s 38 EA chunks (8 chunks, largest `hoc-b048-m002-cross-examination-section-38` 3,255 chars — neither "unfavourable witness" nor "hostile witness" appears in Concepts lines, both terms need injection); Q23 `secondary-chunk-12-execution-of-the-warrant-announcement-requirements-s19-and-s8-search-wa` (1,042 chars, procedure; Concepts currently `s19, s8, announcement, Lippl, O'Neil, show warrant, occupier rights` — missing query-matching terms "search warrant", "execution", "warrant execution", "knock and announce"). Next step: Opus-consulted session to design (a) anchor patch pairs, (b) D1 UPDATE mechanism, (c) selective re-embed via `UPDATE ... SET embedded=0 WHERE id IN (...)`, (d) post-embed baseline A/B. Anchor precision concerns (Priority #6) must be addressed in the same consult — running both questions to Opus together.
...
4. **Vocabulary anchor over-generalisation — architectural concern** — Session 75 Q23 diagnosis surfaced `Misuse of Drugs Act s 29 - Search Powers` chunk winning at 0.6067 on "search warrant execution requirements Tasmania" despite "warrantless search" being semantically opposite to the query. The chunk's session-65 anchor reads `Key terms: search powers, warrantless search, Tasmanian law, police authority, drug possession` — every single query token (search, warrant, Tasmania, requirements) hits the anchor regardless of the "warrantless" antonym. Anchor build heuristic from session 65 treats CONCEPTS list as flat bag-of-words and doesn't discriminate between a chunk's TOPIC vocabulary and its ANTITHESIS vocabulary. This is a latent retrieval risk on every query where a chunk's anchor lifts adjacent-but-wrong content into top-k. Opus consult needed on anchor generation refinement — candidate approaches: (a) separate POSITIVE_TERMS / NEGATIVE_TERMS fields, (b) tighter CONCEPTS linting on ingest, (c) anchor build restricted to title-derived terms only, not Concepts list verbatim. Combine with Priority #1 in same Opus consult.
...
6. **Query expansion** — rewrite user query into 3–4 semantic variants pre-Qdrant via Workers AI Qwen3. Highest long-term ROI on the retrieval side. Build after interleave + aliasing are measured — vocabulary anchors + FTS + interleave may close enough of the gap to deprioritise this.
