...
Updated: 19 April 2026 (end of session 76) · Supersedes all prior versions
Full architecture reference → CLAUDE_arch.md — UPLOAD EVERY SESSION alongside CLAUDE.md
Changelog archive → CLAUDE_changelog.md (sessions 21–65) — load conditionally

---

## SYSTEM STATE — 19 April 2026 (end of session 75)
...
| Baseline (31 queries) | 26P / 3Pa / 2M — unchanged session 76 (19 Apr 2026) post-aliasing + body-injection pass · session 74 canonical `~/retrieval_baseline_post_interleave.txt` remains reference · new snapshots `~/retrieval_baseline_post_aliasing.txt` and `~/retrieval_baseline_post_body_inject.txt` both equal-to-canonical on all 31 queries (MDA s 29 precision fix verified — dropped from Q23 top-3) · generic `~/retrieval_baseline_results.txt` still Apr 16, do not grep |
...
1. **Query expansion — promoted to next retrieval priority** — Session 76 validated experimentally that practitioner↔statutory aliasing cannot be closed by corpus-side edits alone. Additive anchor injection is low-leverage (body dominates vector weight). Body-text prose injection works only when query phrasing overlaps injected wording; natural user phrasings ("hostile witness procedure cross examination", "search warrant execution requirements Tasmania") remain misses despite 6 chunks carrying injected aliases. Query expansion (rewrite user query into 3–4 semantic variants via Workers AI Qwen3 pre-Qdrant, merge results) is the root-cause fix — operates at query side, covers all aliasing pairs uniformly, no per-chunk maintenance. Build: add a variant-generation pre-step in `server.py` before Pass 1, merge across variants with dedup on chunk_id, preserve top score per chunk. Low implementation risk — additive, no schema change, no re-embed.

2. **Authoring gaps — Q10 and Q24 require new doctrine chunks** — D1 verification session 75 returned zero matches for both. Q10: no chunk on EA s 164 (corroboration abolition) / s 165 (unreliability warning) / Longman v The Queen direction — this is why "corroboration warning jury direction" currently surfaces only case applications, no doctrinal anchor. Q24: no chunk on Tas committal procedure — no match on "committal hearing", "preliminary examination", "preliminary proceedings", "hand-up brief", or "s 57A Justices Act". Both need authored and ingested via secondary_sources upload pipeline (pre-formatted block mode, single-chunk if short). Author content using Hogan on Crime where possible; otherwise draft from practitioner knowledge + case law.
...
- **/search top_k=12 server-side cap** — server.py line 296 hard-caps at 12 regardless of requested top_k. Post-session-74 interleave deploy this is no longer blocking for the FTS-new-chunk path. Cap retained for latency bounding. Revisit only if query expansion (Priority #1) generates >12 legitimate candidates per query. Also confirmed session 76: passing `"top_k": 12` in the request payload breaks the endpoint (returns 0 chunks) — the field is not accepted; omit it, default 6 is what the baseline script uses.
- **Q12 / Q23 misses confirmed as query-side aliasing problem, not corpus-side** — Session 76 tested both anchor CONCEPTS injection and body-text prose injection. Anchor-side additive injection: zero baseline movement (body text weight dominates the short anchor prepend). Body-text injection: moves vectors when query phrasing overlaps injected prose (Probe C "cross-examining an unfavourable witness hostile witness common law" → Tendering PIS #1 @ 0.6455 from absent; Probe B "knock and announce warrant" → secondary-chunk-12 #1 @ 0.5251 from #5 @ 0.4997) — but natural user phrasings diverge lexically ("hostile witness procedure cross examination") and don't benefit. Resolution is query-side expansion (Priority #1), not further corpus edits. Q12 + Q23 hold as MISS on the benchmark until query expansion lands, even though corpus-side chunks now carry practitioner aliases in both anchor and body for any query that phrases things close to the injected wording.
