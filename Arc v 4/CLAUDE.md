...
| D1 cases | 1,914 (scraper running daily) · 1,914 deep_enriched=1 |
...
| D1 case_citations | 17,099 rows · subject_matter filter removed session 108 — indexes all deep_enriched=1 cases · cron 0 3 * * * daily, healthy |

| D1 case_legislation_refs | 15,966 rows · source_url backfilled for 5 Acts (Evidence, Criminal Code, Justices, Misuse of Drugs, Police Offences) |

| enrichment_poller | RUNNING — Stage 3 legislation embed complete (26 Acts all embedded=1) · corpus secondary source backlog: 6 (clearing) |
...
- **Word artifact noise** — re-run gen_cleanup_sql.py if new Word-derived chunks ingested (historical cleanups: 131 chunks 18 Mar 2026, 411 rows session 98)
